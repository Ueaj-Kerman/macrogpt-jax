...
from .optimizer_state import TensorRegion, TensorSplitter, map_state
...
class OptimizerConfig:
    ...
    def _collect_param_shapes(self):
        """Collect shapes of all parameters after map_state."""
        self._param_shapes = {}
        params = nnx.state(self.model, nnx.Param)
        
        # Apply map_state to get mapped shapes
        mapped_params = self._apply_map_state(params, from_optimizer=False)
    ...
    def create_optimizer(self, include_state_mapping: bool = True) -> GradientTransformation:
        ...
        When include_state_mapping=True, the optimizer works on mapped tensor shapes.
        Users should specify slices in terms of the mapped coordinates.
        """
    ...
    def _create_masked_optimizer(self, opt: GradientTransformation,
        ...
        If include_state_mapping=True, applies map_state before slicing so that
        slicing happens on mapped coordinates.
        """
        ...
        if include_state_mapping:
            # We need to apply map_state per module
            def init_fn(params):
                # Apply map_state to get mapped params
                mapped_params = self._apply_map_state(params, from_optimizer=False)
                
                # Extract parameters using helper method
                extracted = self._extract_tree_value(mapped_params, access_tree, (), self._extract_param)
                return opt.init(extracted)
            
            def update_fn(updates, state, params=None):
                # Apply map_state to updates and params
                mapped_updates = self._apply_map_state(updates, from_optimizer=False)
                mapped_params = self._apply_map_state(params, from_optimizer=False) if params is not None else None
                
                # Extract updates and params using helper method
                extracted_updates = self._extract_tree_value(mapped_updates, access_tree, (), self._extract_param)
                extracted_params = self._extract_tree_value(mapped_params, access_tree, (), self._extract_param) if mapped_params is not None else None
                
                # Single call to optimizer (on mapped shapes)
                ...
                # Insert updates back into mapped structure
                result = jax.tree.map(zeros_like_preserving_structure, mapped_updates)
                result = self._insert_tree_updates(result, new_updates, access_tree, ())
                
                # Map back to original shape
                final_result = self._apply_map_state(result, from_optimizer=True)
                return final_result, new_state
        else:
            # Original behavior without map_state
            def init_fn(params):
                # Extract parameters using helper method
    ...
    def _apply_map_state(self, state, from_optimizer: bool):
        """Apply map_state transformation to state."""
        # Use the map_state function which properly handles full state trees
        transform = map_state(self.model, from_=from_optimizer)
        result, _ = transform.update(state, None)
        return result
